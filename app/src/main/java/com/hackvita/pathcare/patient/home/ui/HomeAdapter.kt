...
class HomeAdapter(
    var context: Context,
    var hospitals: ArrayList<Hospitals>?
) : RecyclerView.Adapter<HomeAdapter.ViewHolder>(),Serializable {

    class ViewHolder(val binding: HospitalsItemBinding) :
        RecyclerView.ViewHolder(binding.root) {

        fun bindView(hospitalData: Hospitals?, context: Context, position: Int) {

            binding.name.text = hospitalData?.name.toString()
            binding.address.text = hospitalData?.address?.city.toString()
            binding.contactNo.text= hospitalData?.contactNumber?.get(0) ?:"9999999999"
